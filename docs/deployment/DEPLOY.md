...
- HTML routes: `Cache-Control: public, max-age=0, must-revalidate`
- `/assets/*`: `Cache-Control: public, max-age=31536000, immutable`
- `/robots.txt`: `Cache-Control: public, max-age=0, must-revalidate`
- Logo, favicon, Open Graph image, and mascot media:
  `Cache-Control: public, max-age=86400`

Do not add `no-transform` to HTML routes by default. Cloudflare skipped Brotli
compression when `no-transform` was present, and Pages Web Analytics automatic
injection is currently the intended single beacon source. Keep the HTML rules
route-scoped so hashed assets retain a clean one-year cache policy.
...
- **Cloudflare Web Analytics beacon cache TTL.** The repo relies on Cloudflare
  Pages automatic Web Analytics injection as the single beacon source. If a
  duplicate beacon appears in live HTML after deploy, remove any manual beacon
  first; only disable Pages automatic setup if the dashboard-managed script is
  no longer desired.
