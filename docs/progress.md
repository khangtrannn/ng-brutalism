...
**Last action:** Capability-adoption follow-up shipped — IconButton composes tone/radius/shadow/border (removed `NbIconButtonRadius`/`NbIconButtonVariant`; `variant`→`tone`; `md`=`var(--nb-radius)`); MediaItem drops hardcoded hex tone map → `NbToneCapability`/`nbToneVars()`, `NbMediaItemTone`=`NbToneToken`; Chip+Button adopt `NbBorderCapability`; ambiguous `--nb-*-border` normalized to `-border-color`/`-border-width`. New `docs/architecture/capability-discovery.md` records press/focus/disabled/control-size candidates (no generic `NbSizeCapability`). Recipes/docs/tests updated; ui+docs lint/test/build green.
**Next:** `NbPressCapability` (extract Button/IconButton hover-translate), then focus/disabled a11y chapter. Post-1.0: fold Button variant into tone. Then visual QA + resume v0.2.0.
