...
**Last action:** v0.2.0 typography + composition APIs shipped from recipe escape hatches. Shared `tokens/typography.ts` (font-weight, font-role, underline scales). NbDisplay gained `weight`/`fluid`/`underlineGap`/`underlineWidth`; NbText gained `underlineGap`/`underlineWidth` (both reuse shared weight scale). New `NbTypography` directive + `nbSurface typography="…"` font-role context (tokens `--nb-font-{body,display,accent,mono}`). NbChipGroup now owns layout + chip context (radius/shadow/transform/tracking). NbMediaItem `size="xs"`; title font-family now inherits. Recipe icons standardized on `--nb-icon-size`. NbIconTile deferred (pattern only 2× in one recipe — see roadmap). 4 recipes refactored; ui+docs build green.
