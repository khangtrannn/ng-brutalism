...
**Last action:** Button tone normalization shipped — `variant` + `NbButtonVariant` removed (no alias), Button composes `NbToneCapability` so `tone` is the single color axis (default `primary`, replacing bespoke `--nb-main`); color flows through `--nb-button-{bg,fg,border-color}` (manual `toneBg()`/`toneFg()` deleted). `shadow`/press stays local. Docs/recipes/tests updated (variant→tone; neutral→`tone="background"`); ui+docs lint/test/build green.
