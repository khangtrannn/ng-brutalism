...
**Last action:** API language cleanup shipped — `size` middle rung `default`→`md` (IconButton/Display/Input/Textarea/Checkbox); layout child `divider`→`separator` (Stack/Cluster/Split; `NbStackSeparator` etc. + `--nb-cluster-separator-*` + `data-separator`); Section keeps `divider`/`dividerStyle` placement; removed Button `fontSize`/`weight`/`transform`/`tracking` inputs+types (default `font-bold` kept; expressive labels use nested `nbText`). Recipes/docs updated; no compat aliases. ui+docs lint/test/build green.
**Next:** Capability-adoption wave: widen tone/radius/shadow/border to IconButton, MediaItem tone capability + kill hex map, delete `NbIconButtonRadius`, Chip/Button/IconButton border capability, `NbPressCapability`. Post-1.0: fold Button variant into tone. Then visual QA + resume v0.2.0.
